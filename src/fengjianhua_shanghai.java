...
public class fengjianhua_shanghai {

	int K = 60;//
	int I = 50;//
	int iterNum = 600;//1000
	...
	Map<Integer,List<String>> negativeSet;
	...
	fengDataset_shanghai ds;
	
	public fengjianhua_shanghai(int K,int I)
	{
		this.K = K;
		this.I = I;
		ds= new fengDataset_shanghai();
		//utrain=ds.utrain;
		trainset=ds.trainset;
		testset=ds.testset;
		negativeSet=ds.negativeSet;
...
//					}
			...
			for(int j=0;j<U;j++)
			{
				...
				int[] index = ArrayUtils.argsort(score[j],false);//ArrayUtils.argsort 用于对数组进行排序，并返回排序后元素的索引
				int[] array = new int[topn];
				for (int g=0;g<topn;g++) {
					System.out.println("score[j][g]="+score[j][index[g]]);
				}
	...
	public Map<Integer,String[]> recommendori2(fengmodel model,int topn,String inout)
	{
		//
		nuz=null;
		nzi=null;
		...
		System.out.println("generating recommendation...");
		Map<Integer,int[]> reclist = new HashMap<Integer,int[]>();
		Map<Integer,String[]> reclist_score = new HashMap<Integer,String[]>();
...
//									(1-model.lambdaU[uid])*model.phiZI[k][j]*model.phiIX[j][x]*model.phiIY[j][y]*model.phiIO[j][o]);
			...
			{
//
//				double[] score2 = new double[score[j].length];
//				for (int o=0; o<score[j].length; o ++) {
//					System.out.println("score[j][o]=" + score[j][o]);
//					BigDecimal big = new BigDecimal(score[j][o]);
//					System.out.println("big=" + big);
//					BigDecimal temp = big.setScale(10,2);
//					System.out.println("temp=" + temp);
//					score2[o] = temp.doubleValue(); 
//					System.out.println("score2[o]=" + score2[o]);
//				}
//				
				int[] index = ArrayUtils.argsort(score[j],false);//ArrayUtils.argsort 用于对数组进行排序，并返回排序后元素的索引
//				
...
//				}
//				System.out.println("************************");
//				int[] index2 = ArrayUtils.argsort(ss,false);
//				for (int g=0;g<500;g++) {
//					System.out.println("ss[index2[g]]=" + ss[index2[g]]);
//				}
			
				String[] array2 = new String[topn];
	
				for(int g=0;g<topn;g++) {
					

					double s3 = score[j][index[g]];
					System.out.println("score[j][index[g]]="+s3);
					array2[g]=index[g]+"+"+s3;

				}
				reclist_score.put(j, array2);

			}

			return reclist_score;
			
		}		
		return null;
	}
	
	public Map<Integer,String[]> recommendori3(fengmodel model,int topn,String inout)
	{
		//
		nuz=null;
		nzi=null;
		...
		double[][] score = new double[U][100];
		int k;
		for(int uid:negativeSet.keySet())
		{
			System.out.println(uid);
			List<Integer> candlist = new ArrayList<Integer>();
				
			for (String negativeId : negativeSet.get(uid)) {
				candlist.add(Integer.parseInt(negativeId));
			}
	...
	public void saveRecList2(String recfile,String recfilenorm, Map<Integer,String[]> list) {
		String[] emap = new String[E];
		//
		System.out.println("write");
		for(Map.Entry<String, Integer> entry : ds.emap.entrySet()){
...
//				System.out.println("Y="+Y);
				fengjianhua_shanghai jh = new fengjianhua_shanghai(K,Y);
				jh.initialize();
...
//				Map<Integer,String[]> lists2 = jh.recommendori2(model, topn, matrix);
				Map<Integer,String[]> lists3 = jh.recommendori3(model, topn, matrix);
				
				String recfile="fengresult/shanghai/iteration/reccateK"+K+"Y"+Y+"_600.txt";//
				String recfilenorm="fengresult/shanghai/iteration/reccateK"+K+"Y"+Y+"norm_600.txt";
				
//				String recfile2="fengresult/shanghai/iteration/reccateK"+K+"Y"+Y+"_600+score.txt";//
//				String recfilenorm2="fengresult/shanghai/iteration/reccateK"+K+"Y"+Y+"norm_600+score.txt";
				
				String recfile3="fengresult/shanghai/iteration/reccateK"+K+"Y"+Y+"_600_negative.txt";//
				String recfilenorm3="fengresult/shanghai/iteration/reccateK"+K+"Y"+Y+"norm_600_negative.txt";
...
//				***********************reclist + score*****************************
//				File file2 = new File(recfile2);
//				if(file2.exists())
//				{
//					file2.delete();
//					jh.saveRecList2(recfile2,recfilenorm2,lists2);
//				}
//				else
//				{
//					jh.saveRecList2(recfile2,recfilenorm2,lists2);
...
//				***********************negative + score*****************************
				File file3 = new File(recfile3);
				if(file3.exists())
				{
					file3.delete();
					jh.saveRecList2(recfile3,recfilenorm3,lists3);
				}
				else
				{
					jh.saveRecList2(recfile3,recfilenorm3,lists3);
